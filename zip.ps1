...
# 古いZIPファイルを削除
if (Test-Path "DarkBlueThemeX.zip") {
    Remove-Item "DarkBlueThemeX.zip" -Force
...
Compress-Archive -Path "$tempDir/*" -DestinationPath "DarkBlueThemeX.zip" -Force

# 一時ディレクトリを削除
Remove-Item $tempDir -Recurse -Force

if (Test-Path "DarkBlueThemeX.zip") {
    Write-Host "ZIPファイルを作成しました: DarkBlueThemeX.zip" -ForegroundColor Green
    Write-Host ""
    Write-Host "ファイルサイズ:" -ForegroundColor Cyan
    $fileSize = (Get-Item "DarkBlueThemeX.zip").Length
